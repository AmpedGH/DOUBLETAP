...
# Runs airodump-ng in the foreground so its live table is visible.
# Press Ctrl+C to stop the scan and continue; this does NOT exit the
# whole script, since SIGINT is temporarily ignored by the shell while
# airodump-ng owns the terminal.
run_scan() {
    local prefix="$1"
    shift
    local extra_args=("$@")
    ...
    echo
    echo "[*] Starting airodump-ng — live view below."
    echo "[*] Press Ctrl+C when you're ready to stop this scan and continue."
    rm -f "${prefix}"-*.csv 2>/dev/null

    # Let Ctrl+C stop only airodump-ng (foreground), not the whole script
    trap '' INT
